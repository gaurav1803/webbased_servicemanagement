...
//if ($_SERVER["REQUEST_METHOD"] == "POST") {
  //      $doctorName=$_POST["doctorName"];
    ...
    //    }
  $selectSQL = "SELECT name,age,specialization,city,experience FROM doctor_info";
  //$doctorName="";
    ...
    <?php
      ...
      if( mysqli_num_rows( $selectRes )==0 ){
        ...
        while( $row = mysqli_fetch_assoc( $selectRes ) ){
          echo "<tr><td>{$row['name']}</td> <td>{$row['age']}</td> <td>{$row['specialization']}</td><td>{$row['city']}</td><td>{$row['experience']}</td></tr>\n";
        }
      }
    // }
    ?>
